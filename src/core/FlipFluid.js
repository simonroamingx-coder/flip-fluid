...
//
// FLIP fluid solver. This is a faithful port of the simulator in 18-flip.html:
// the math, the array layouts, the iteration order and the constants are
// unchanged, so a given input state produces byte-identical output.
//
// The only edits are structural:
//   1. updateParticleDensity() read a global `f`; it now reads `this`.
//   2. handleParticleCollisions() read the global `scene`; the obstacle
//      velocity is now a parameter.
//   3. Unused locals and unreachable comments were dropped.
    ...
    updateParticleColors()
    {
        ...
        for (var i = 0; i < this.numParticles; i++) {
            ...
            if (d0 > 0.0) {
                ...
                if (relDensity < 0.7) {
                    var sc = 0.8;
                    this.particleColor[3 * i] = sc;
                    this.particleColor[3 * i + 1] = sc;
                    this.particleColor[3 * i + 2] = 1.0;
                }
            }
        }
